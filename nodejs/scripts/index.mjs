...
async function buildWasmPipeline({ rustPath, wasmOutPath }) {
  ...
  const wasmCrates = await getLocalRustCrates({ rustPath })
...
async function buildWasmCrate({ wasmCrate, profile, rustPath, wasmOutPath }) {
  ...
  await runCommand({
    cmd: 'wasm-bindgen',
    args: ['--target', 'bundler', '--out-dir', wasmOutPath, path.join('target', wasmTarget, rustProfileToFolder(profile), `${wasmCrateName}.wasm`)],
    path: rustPath,
  })
}
